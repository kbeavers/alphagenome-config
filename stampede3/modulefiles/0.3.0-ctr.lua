...
on Stampede3 (H100 nodes, x86_64). It exposes:
...
whatis("Description: AlphaGenome run environment using a TACC container image (Stampede3/H100).")
...
-- the image's own CUDA libraries. --nv still injects the GPU driver. We re-pass
-- only what's needed; CUDA_VISIBLE_DEVICES is forwarded when SLURM sets it.
